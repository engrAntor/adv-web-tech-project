...
import { Course } from './courses.entity';  // Fix the import path
import { User } from '../users/users.entity';  // Fix the import path
...
export class CoursesController {
  ...
  async getProgress(@Param('userId') userId: number): Promise<Progress[]> {
    const user = await this.coursesService.findUserById(userId);  // This will be defined in the service
