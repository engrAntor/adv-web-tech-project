...
export class UsersController {
  ...
  async updateProfile(
    @Request() req: any,
    @Body() body: { firstName?: string; lastName?: string; bio?: string; phone?: string },
  ) {
  ...
  async updatePassword(
    ...
    await this.usersService.updatePassword(req.user.id, body.currentPassword, body.newPassword);
  ...
  async findOne(@Param('id') id: number) {
    ...
    if (user) {
      const { password, passwordResetToken, emailVerificationToken, passwordResetExpires, ...result } = user;
      return result;
    }
    return null;
  }
