...
export class PaymentsController {
  ...
  async confirmBkashPayment(@Body() dto: ConfirmPaymentDto) {
    return this.paymentsService.confirmBkashPayment(
      dto.transactionId,
      dto.bkashTransactionId!,
    );
  }
  ...
  async checkCoupon(@Request() req: any, @Body() dto: ApplyCouponDto) {
    return this.paymentsService.checkCoupon(
      dto.couponCode,
      dto.courseId,
      req.user.id,
    );
  ...
  async getPaymentByTransaction(@Param("transactionId") transactionId: string) {
    const payment =
      await this.paymentsService.getPaymentByTransactionId(transactionId);
    if (!payment) {
  ...
  async getInvoiceByNumber(@Param("invoiceNumber") invoiceNumber: string) {
    const invoice =
      await this.paymentsService.getInvoiceByNumber(invoiceNumber);
    if (!invoice) {
      throw new NotFoundException("Invoice not found");
    }
    return invoice;
  ...
  async updateCoupon(
    @Param("id") id: number,
    @Body() data: Partial<CreateCouponDto>,
  ) {
