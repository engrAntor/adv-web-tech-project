...
import { Payment, PaymentMethod, PaymentStatus, Currency } from './payment.entity';
import { Coupon, DiscountType } from './coupon.entity';
import { Invoice } from './invoice.entity';
import { User } from '../users/users.entity';
import { Course } from '../courses/courses.entity';
import { Enrollment, PaymentStatus as EnrollmentPaymentStatus } from '../enrollments/enrollment.entity';
...
export class PaymentsService {
  ...
  constructor(
    ...
    this.stripe = new Stripe(process.env.STRIPE_SECRET_KEY || 'sk_test_51RemQB2QRDj0NicDV25YNpfCjSncoQCFzBwMtzlSkD4GnhwzNLV8Z8wYkKywePqAAOMhmJPiXzQQL1wW97BCe20200cfqaarIb', {
  ...
  async initiatePayment(userId: number, dto: InitiatePaymentDto): Promise<Payment> {
    ...
    if (dto.couponCode) {
      const couponResult = await this.validateAndApplyCoupon(dto.couponCode, course.id, userId, amount);
    ...
    if (dto.paymentMethod === PaymentMethod.STRIPE) {
      const paymentIntent = await this.createStripePaymentIntent(finalAmount, currency, user.email, course.title);
      ...
      const amountInBDT = currency === Currency.BDT ? finalAmount : finalAmount * this.USD_TO_BDT_RATE;
      ...
      const amountInBDT = currency === Currency.BDT ? finalAmount : finalAmount * this.USD_TO_BDT_RATE;
      const paymentIntent = await this.createStripePaymentIntent(amountInBDT, Currency.BDT, user.email, course.title);
      payment.stripePaymentIntentId = paymentIntent.id;
      payment.stripeClientSecret = paymentIntent.client_secret || undefined;
  ...
  private async createStripePaymentIntent(amount: number, currency: Currency, email: string, description: string): Promise<Stripe.PaymentIntent> {
  ...
  async confirmStripePayment(transactionId: string, stripePaymentIntentId: string): Promise<Payment> {
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      // Update coupon usage
      if (payment.couponId) {
        await this.couponRepository.increment({ id: payment.couponId }, 'usedCount', 1);
      }
  ...
  async confirmBkashPayment(transactionId: string, bkashTransactionId: string): Promise<Payment> {
    const payment = await this.paymentRepository.findOne({
      where: { transactionId },
      relations: ['user', 'course'],
    });
  ...
  private async handleFreeEnrollment(userId: number, course: Course, couponCode?: string): Promise<Payment> {
    const payment = this.paymentRepository.create({
      transactionId: this.generateTransactionId(),
      userId,
      courseId: course.id,
  ...
  private async createEnrollmentAfterPayment(payment: Payment): Promise<void> {
    ...
    await this.courseRepository.increment({ id: payment.courseId }, 'enrollmentCount', 1);
  }
  ...
  private async createInvoice(payment: Payment): Promise<Invoice> {
    ...
    const course = await this.courseRepository.findOneBy({ id: payment.courseId });
  ...
  async validateAndApplyCoupon(
    ...
    if (coupon.minPurchaseAmount && originalAmount < Number(coupon.minPurchaseAmount)) {
  ...
  async checkCoupon(couponCode: string, courseId: number, userId: number): Promise<{
    valid: boolean;
    discount?: number;
    ...
    try {
      ...
      const result = await this.validateAndApplyCoupon(couponCode, courseId, userId, Number(course.price));
  ...
  async getAllPayments(limit = 20, offset = 0): Promise<{ payments: Payment[]; total: number }> {
