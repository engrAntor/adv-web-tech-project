...
export class CertificatesService {
  ...
  async createCertificate(userId: number, courseId: number): Promise<Certificate> {
    ...
    if (!progress || progress.status !== ProgressStatus.COMPLETED) {
      throw new BadRequestException('Course must be completed to receive a certificate');
    }
  ...
  async generateCertificatePDF(certificateId: number): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Top-right corner
      doc.moveTo(pageWidth - 50 - cornerSize, 50).lineTo(pageWidth - 50, 50).lineTo(pageWidth - 50, 50 + cornerSize).stroke();
      // Bottom-left corner
      doc.moveTo(50, pageHeight - 50 - cornerSize).lineTo(50, pageHeight - 50).lineTo(50 + cornerSize, pageHeight - 50).stroke();
      // Bottom-right corner
      doc.moveTo(pageWidth - 50 - cornerSize, pageHeight - 50).lineTo(pageWidth - 50, pageHeight - 50).lineTo(pageWidth - 50, pageHeight - 50 - cornerSize).stroke();
      ...
      // Student Name
      const studentName = certificate.studentName || certificate.user?.email || 'Student';
      doc.fillColor('#1e40af')
         .fontSize(32)
         .font('Helvetica-Bold')
      ...
      doc.fillColor('#1e3a5f')
         .fontSize(24)
         .font('Helvetica-Bold')
         .text(certificate.course.title, 50, 360, { align: 'center', width: pageWidth - 100 });
      ...
      doc.fillColor('#94a3b8')
         .fontSize(10)
         .font('Helvetica')
         .text(`Certificate ID: ${certificate.certificateCode}`, 0, 450, { align: 'center' });

      // Verification URL
      doc.fillColor('#3b82f6')
         .fontSize(9)
         .font('Helvetica')
         .text(`Verify at: ${process.env.FRONTEND_URL || 'http://localhost:3000'}/certificates/verify?code=${certificate.certificateCode}`, 0, 470, { align: 'center' });
      ...
      doc.fillColor('#475569')
         .fontSize(10)
         .font('Helvetica')
         .text('LMS Platform', leftSignatureX, signatureY + 10, { width: 150, align: 'center' });
      ...
      const instructorName = certificate.course.instructor
        ? `${certificate.course.instructor.firstName || ''} ${certificate.course.instructor.lastName || ''}`.trim() || 'Course Instructor'
        : 'Course Instructor';

      doc.fillColor('#475569')
         .fontSize(10)
         .font('Helvetica')
         .text(instructorName, rightSignatureX, signatureY + 10, { width: 150, align: 'center' });
  ...
  async findByCodePublic(certificateCode: string): Promise<{
    ...
    return {
      id: certificate.id,
      certificateCode: certificate.certificateCode,
      studentName: certificate.studentName || certificate.user?.email || 'Student',
      courseName: certificate.course.title,
      courseDescription: certificate.course.description,
      issuedAt: certificate.issuedAt,
      isValid: !isExpired,
