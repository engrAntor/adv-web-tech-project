...
async function bootstrap() {
    ...
    if (fs.existsSync(certPath)) {
      const httpsOptions = {
        ...
        cert: fs.readFileSync(
          join(process.cwd(), "certificates", "localhost.pem"),
        ),
      };
  ...
  // CORS configuration
  const allowedOrigins = ["http://localhost:3000", "https://localhost:3000", "http://localhost:3001", "https://localhost:3001"];
  ...
  app.enableCors({
    origin: isProduction
      ? process.env.CORS_ORIGIN || allowedOrigins
      : allowedOrigins,
