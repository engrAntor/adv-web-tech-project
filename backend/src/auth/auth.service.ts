...
export class AuthService {
  ...
  async register(email: string, password: string, firstName?: string, lastName?: string): Promise<any> {
    ...
    if (firstName || lastName) {
      await this.usersService.updateProfile(newUser.id, { firstName, lastName });
    }
  ...
  async forgotPassword(email: string): Promise<{ message: string }> {
    try {
      const { otp, user } = await this.usersService.setPasswordResetOTP(email);
  ...
  async verifyPasswordResetOTP(email: string, otp: string): Promise<{ token: string; message: string }> {
